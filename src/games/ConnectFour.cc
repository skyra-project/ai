...
namespace connect_four
{
	...
	void calculate_remaining(ai_board &board)
	{
	...
	bool compare(Players a, Players b, Players c, Players d) noexcept
	{
		return a == b && b == c && c == d;
	}

	bool check(const ai_board &board, size_t cell,
			   int_fast8_t a, int_fast8_t b, int_fast8_t c,
			   int_fast8_t d) noexcept
	{
	...
	bool check(const ai_board &board, size_t cell,
			   int_fast8_t a, int_fast8_t b, int_fast8_t c,
			   int_fast8_t d, int_fast8_t e) noexcept
	{
		const auto ca = board.cells[cell + a];
		const auto cb = board.cells[cell + b];
		const auto cc = board.cells[cell + c];
	...
	bool check(const ai_board &board, size_t cell,
			   int_fast8_t a, int_fast8_t b, int_fast8_t c,
			   int_fast8_t d, int_fast8_t e, int_fast8_t f) noexcept
	{
		const auto ca = board.cells[cell + a];
		const auto cb = board.cells[cell + b];
		const auto cc = board.cells[cell + c];
	...
	bool check(const ai_board &board, size_t cell,
			   int_fast8_t a, int_fast8_t b, int_fast8_t c,
			   int_fast8_t d, int_fast8_t e, int_fast8_t f,
			   int_fast8_t g) noexcept
	{
		const auto ca = board.cells[cell + a];
		const auto cb = board.cells[cell + b];
		const auto cc = board.cells[cell + c];
	...
	bool status_row(const ai_board &board, uint_fast8_t mask, size_t cell,
					int_fast8_t l1, int_fast8_t l2, int_fast8_t l3,
	...
	bool column_available(const ai_board &board, int_fast8_t position)
	{
		return board.remaining[position] != static_cast<uint_fast8_t>(0);
	}

	size_t piece_offset(ai_board &board, int_fast8_t column)
	{
		return static_cast<size_t>(((board.remaining[column] - 1) * board_width) + column);
	}

	void piece_add(ai_board &board, int_fast8_t column, size_t offset, Players player)
	{
		--board.remaining[column];
		board.cells[offset] = player;
	}

	void piece_remove(ai_board &board, int_fast8_t column, size_t offset)
	{
		++board.remaining[column];
		board.cells[offset] = Players::Unset;
	}

	ai_results min(ai_board &board, size_t last_move, int_fast8_t remaining, int_fast8_t alpha, int_fast8_t beta) noexcept;
	ai_results max(ai_board &board, size_t last_move, int_fast8_t remaining, int_fast8_t alpha, int_fast8_t beta) noexcept;
	...
	int_fast8_t position(ai_board &board, int_fast8_t remaining, int_fast8_t maximum_depth) noexcept
	{
		...
		return max(board, undefined_last_move, std::min(remaining, maximum_depth), alpha, beta)
			.position;
	}
} // namespace connect_four
